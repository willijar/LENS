...
(defclass event()
  ((rank :type fixnum :initform -1
   ...
   (priority :type fixnum :initform 0 :initarg :priority :reader priority
             :documentation "Determines delivery of messages with same
             arrival time")
   (schedule-id :initform -1 :type integer
             :documentation "Used to ensure events with
   same time and priority are scheduled in order of scheduling" :reader uid)
  ...
  (:method((delay number) (event event))
    ...
    (let ((scheduler (scheduler)))
      (setf (slot-value event 'event-time) (+ delay (simulation-time))
            ...
            (incf (slot-value scheduler 'last-schedule-id)))
